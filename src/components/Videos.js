...
const Videos = () => {
    const { course } = useParams(); // Use useParams to get the course from the URL
    ...
    const selectedVideos = videoLinks[course];

    return (
        <div>
            <h2>Videos for {course.replace(/-/g, ' ')}</h2>
            {selectedVideos && selectedVideos.length > 0 ? (
                <div className="video-gallery">
                    {selectedVideos.map((video, index) => (
                        <div className="video-item" key={index}>
                            <a href={video.url} target="_blank" rel="noopener noreferrer">
                                <img src={video.img} alt={`Video ${index + 1}`} className="video-thumbnail" />
                            </a>
                        </div>
                    ))}
                </div>
